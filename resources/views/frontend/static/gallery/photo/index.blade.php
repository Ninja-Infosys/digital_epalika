@extends('frontend.layouts.master')
@section('content')
<section class="photo-section">
    <div class="container">
        <div class="row">
            <div class="col-lg-3 mt-3">
                <h6 class="font-weight-bold mb-md-3">ग्यालेरी</h6>
                <div class="sidebar">
                    <div class="row">
                        <div class="col-sm-6 col-md-4 col-lg-12 mt-2 mt-sm-0 mt-lg-2">
                            <a routerlink="/static/audio" routerlinkactive="active" href="#static/audio">
                                <div class="card-06">
                                    <h6 class="f1">अडियो ग्यालेरी</h6>
                                </div>
                            </a>
                        </div>
                        <div class="col-sm-6 col-md-4 col-lg-12 mt-2 mt-sm-0 mt-lg-2">
                            <a routerlink="/static/video" routerlinkactive="active" href="#static/video">
                                <div class="card-06">
                                    <h6 class="f1">भिडियो ग्यालेरी</h6>
                                </div>
                            </a>
                        </div>
                        <div class="col-sm-6 col-md-4 col-lg-12 mt-2 mt-sm-0 mt-lg-2">
                            <a routerlink="/static/photo" routerlinkactive="active" href="#static/photo" class="active">
                                <div class="card-06">
                                    <h6 class="f1">फोटो ग्यालेरी</h6>
                                </div>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-9 mt-3">
                <h4 class="title-dark mb-3">फोटो ग्यालेरी</h4>
                <div class="row">
                    <div class="col-12 col-sm-6 col-md-4 mb-4">
                        <a class="album" [routerLink]="['/static/single-photo/', 'test']">
                            <img class="album-img" src="assets/img/mou.jpg">
                            <h6 class="album-title">नेपालगंज उप-महानगरपालिकाको संक्षिप्त परिचय तस्बिरहरु</h6>
                            <div class="d-flex"><span class="count">2 Items</span></div>
                        </a>
                    </div>
                    <div class="col-12 col-sm-6 col-md-4 mb-4">
                        <a class="album" [routerLink]="['/static/single-photo/', 'test']">
                            <img class="album-img" src="assets/img/mou.jpg">
                            <h6 class="album-title">नेपालगंज उप-महानगरपालिकाको संक्षिप्त परिचय तस्बिरहरु</h6>
                            <div class="d-flex"><span class="count">4 Items</span></div>
                        </a>
                    </div>
                    <div class="col-12 col-sm-6 col-md-4 mb-4">
                        <a class="album" [routerLink]="['/static/single-photo/', 'test']">
                            <img class="album-img" src="assets/img/mou.jpg">
                            <h6 class="album-title">नेपालगंज उप-महानगरपालिकाको संक्षिप्त परिचय तस्बिरहरु</h6>
                            <div class="d-flex"><span class="count">4 Items</span></div>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
    @push('style')
        <link rel="stylesheet" href="{{asset('assets/frontend/css/gallery/photo.css')}}">
    @endpush
@endsection
