        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between">
                   ...
                   <div class="d-flex justify-content-between"> <a href="javascript:void(0)"
                            route_action="{{ route('emap.admin.building-documentation.print-template',[$buildingDocumentation,$form,$buildingFormDataType]) }}" class="btn btn-primary btn-sm printDetail">
            ...
            <div class="card-body">
                @if(count($buildingDocumentation->buildingDocuments->where('form_data_id', $buildingFormDataType->id)) == 0)
                    <form
                        action="{{ route('emap.admin.building-documentation.storeDocument', [$buildingDocumentation, $form, $buildingFormDataType]) }}"
                ...
                @else
                    <form
                        action="{{ route('emap.admin.building-documentation.updateDocument', [$buildingDocumentation, $form, $buildingFormDataType, $buildingDocumentation->buildingDocuments->where('building_documentation_step_id', $form->id)->sortByDesc('created_at')?->first()?->id]) }}"
                        method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                        ...
                        @foreach($buildingDocumentation->buildingDocuments?->load('documentFiles','documentStatuses')?->where('form_data_id', $buildingFormDataType->id) as $buildingDocument)
                            ...
                            @foreach ($buildingDocument->documentStatuses?->load('documentFiles') as $status)
                                <tr style="background-color: #e8e5e5;">
                                    ...
                                    <td>    @foreach ($status->documentFiles as $statusFile)
                                            <button type="button" class="btn btn-info" data-bs-toggle="modal"
