                                ...
                                @forelse($branches as $key=>$branch)
                                    <tr>
                                        ...
                                        <td class="d-flex gap-1">
                                            @can('branch_edit')
                                                <a data-bs-type="edit"
                                                    href="{{ route('admin.global.generalSetting.branch.edit', $branch) }}"
                                            ...
                                            @can('branch_delete')
                                                <form
                                                    action="{{ route('admin.global.generalSetting.branch.destroy', $branch) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('delete')
                                    ...
                                    @foreach ($branch->branches as $subBranch)
                                        <tr>
                                            ...
                                            <td>
                                                @can('branch_edit')
                                                    <a data-bs-type="edit"
                                                        href="{{ route('admin.global.generalSetting.branch.edit', $subBranch) }}"
                                                        title="सम्पादन गर्नुहोस्"
                                                        class="btn btn-xs btn-outline-primary {{ get_setting('Pin') ? 'confirm_pin' : '' }}">
                                                ...
                                                @can('branch_delete')
                                                    <form
                                                        action="{{ route('admin.global.generalSetting.branch.destroy', $subBranch) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('delete')
                                                        <button data-bs-type="delete"
