            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between">
                        <h4 class="header-title">गुनासो सुन्ने अधिकारी </h4>
                    </div>
                </div>
                <div class="card-body">
                    <form
                        action="{{route('admin.grievanceHandling.setting.grievanceSetting.update',$grievanceSetting)}}"
                        method="post"
                            ...
                            <div class="col-md-12 mb-2">
                                ...
                                <select name="user_id" class="form-control" id="user_id">
                                    ...
                                    @foreach($users as $user)
                                        <option
                                            value="{{$user->id}}" {{old('user_id',$grievanceSetting->user_id)==$user->id ? 'selected':''}}>{{$user->name}}</option>
                                    @endforeach
                                </select>
                                @error('user_id')
