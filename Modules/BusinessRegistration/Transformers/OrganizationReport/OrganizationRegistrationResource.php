...
namespace Modules\BusinessRegistration\Transformers\OrganizationReport;

use Illuminate\Http\Resources\Json\JsonResource;
use Modules\BusinessRegistration\Transformers\OrganizationReport\OrganizationRenewResource;
use Modules\BusinessRegistration\Transformers\Report\PartnerResource;

class OrganizationRegistrationResource extends JsonResource
{
    public function toArray($request): array
    {
        $request_columns = $request->input('columns')['organization_registrations'] ?? [];
        return [
            ...
            'आर्थिक स्रोत' => $this->when(in_array('financial_source', $request_columns), $this->financial_source ?? ''),
            ...
            'कर तिरेको प्रमाणपत्र नं.' => $this->when(in_array('taxpayer_number', $request_columns), $this->taxpayer_number ?? ''),
            'व्यवसायी विवरण' => PartnerResource::collection($this->whenLoaded('partners')),

            'व्यवसाय नवीकरण' => OrganizationRenewResource::collection($this->whenLoaded('organizationRenew')),
        ];
    }
