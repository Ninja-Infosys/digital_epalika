...
return new class () extends Migration {
    ...
    {
        Schema::create('organization_renews', function (Blueprint $table) {
            $table->id();
            $table->foreignId('fiscal_year_id')->comment('आर्थिक बर्ष')->constrained()->cascadeOnDelete();
            $table->foreignId('organization_registration_id')->constrained()->cascadeOnDelete();
            $table->string('date')->comment('नबिकरण मिति वि.सं.');
            $table->string('date_en')->comment('नबिकरण मिति ई.सं.');
    ...
    {
        Schema::dropIfExists('organization_renews');
    }
};
