...
class BuildingHouseOwner extends Model
{
    use HasFactory;
    use SoftDeletes;
    ...
    public function setDocumentAttribute($value): void
    {
        if (! empty($value) && ! is_string($value)) {
            $this->attributes['document'] = $value->store('buildingHouseOwnerArchive', 'public');
        }
    }
    ...
    {
        if (! empty($value) && ! is_string($value)) {
            $this->attributes['photo'] = $value->store('e_map/buildingDocumentation/house_owner/photo', 'public');
        }
    }
    ...
    {
        if (! empty($value) && ! is_string($value)) {
            $this->attributes['signature'] = $value->store('e_map/buildingDocumentation/house_owner/signature', 'public');
        }
    }
