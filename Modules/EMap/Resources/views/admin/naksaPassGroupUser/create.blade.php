    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('emap.admin.enaksa.naksaPassGroupUser.store')}}" method="post">
                        @csrf
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                        <label for="user_id">आर्थिक बर्ष</label>
                                        <select name="user_id[]" multiple data-toggle="select2"
                                    ...
                                    <div class="col-md-6 mb-2">
                                        <label for="group_id">आर्थिक बर्ष</label>
                                        <select name="group_id[]" multiple data-toggle="select2"
                                                id="group_id" class="form-control">
                                            <option disabled>--- छान्नुहोस् ---</option>
                                            @foreach($naksaPassGroups as $naksaPassGroup)
                                            <option value="{{$naksaPassGroup->id}}" {{ in_array($naksaPassGroup->id, old('group_id', [])) ? 'selected' : '' }}>{{$naksaPassGroup->title}}<option>
                                            @endforeach
                                        </select>
                                        @error('group_id')
                                        <div class="invalid-feedback">{{$message}}</div>
