                ...
                <div class="card-body">
                    <form action="{{ route('emap.admin.mapPassGroup.update', $mapPassGroup) }}" method="post">
                        ...
                        @foreach ($users as $index => $user)
                            <div class="col-md-12 mb-2">
                                <label class="form-label">प्रयोगकर्ता {{ $user->name }}</label>
                                <div class="row">
                                    <div class="col-md-3 d-flex">
                                        <input type="checkbox" name="users[{{ $index }}][user_id]" value="{{ $user->id }}"
                                            class="form-check @error('users.' . $index . '.user_id') is-invalid @enderror me-1"
                                ...
                                <div class="col-md-3 mb-2">
                                    ...
                                    <select name="users[{{ $index }}][ward_no][]" class="form-select" multiple>
                                        ...
                                        @foreach (officeSetting()->localBody->ward_no as $ward)
                                            <option value="{{ $ward }}" {{ in_array($ward, old("users.$index.ward_no", explode(',', DB::table('map_pass_group_user')->where('map_pass_group_id', $mapPassGroup->id)->where('user_id', $user->id)->first()->ward_no ?? ''))) ? 'selected' : '' }}>
                                                {{ $ward }}
                                            </option>
                                        @endforeach
                                    </select>
