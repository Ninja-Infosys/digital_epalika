    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <form method="post" action="{{route('emap.admin.form.update',$form)}}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-2">
                                ...
                                <select id="form_type" name="form_type"
                                    ...
                                    @foreach(\Modules\EMap\Enums\FormTypeEnum::cases() as $formType)
                                        <option
                                            value="{{$formType->value}}" {{old('form_type', $form->form_type->value) == $formType->value ? 'selected' : ''}}>{{$formType->label()}}</option>
                                    @endforeach
                            ...
                            <div class="col-md-4 mb-2">
                                ...
                                <select id="map_pass_group_id" name="map_pass_group_id"
                                    ...
                                    @foreach($mapPassGroups as $mapPassGroup)
                                        <option
                                            value="{{$mapPassGroup->id}}" {{old('map_pass_group_id', $form->map_pass_group_id) == $mapPassGroup->id ? 'selected' : ''}}>{{$mapPassGroup->title}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4 mb-2">
                                ...
                                <select id="need_from" name="need_from"
                                    ...
                                    @foreach(\Modules\EMap\Enums\EMapFormFillerTypeEnum::cases() as $needFrom)
                                        <option
                                            value="{{$needFrom->value}}" {{old('need_from', $form->need_from->value) == $needFrom->value ? 'selected' : ''}}>{{$needFrom->label()}}</option>
                                    @endforeach
                            ...
                            <div class="col-md-4 mb-2" id="form">
                                ...
                                <div class="d-flex justify-content-between gap-1">
                                    <input type="text" class="form-control personalDetail"
                                           value="{{old('route_name', $form->route_name)}}"
                            ...
                            <div class="col-md-12 mb-2" id="file">
                                ...
                                <fieldset class="bg-soft-secondary">
                                    <div id="files">
                                        @foreach(old('fields', $form->formDocumentFormats ?? []) as $index=>$field)
                                            <div class="main">
                                                <div class="text-end">
                                                ...
                                                <div class="row border-bottom mb-2">
                                                    <div class="col-md-12 mb-2">
                                                        <label for="documents" class="form-label">शिर्षक </label>
                                                        <input type="text" name="fields[{{$index}}][title]"
                                                               value="{{old('fields.'.$index.'.title', $field->title)}}"
                                                               class="form-control"
                                                               id="documents" multiple/>
                                                    </div>

                                                    <div class="col-md-12 mb-2">
                                                        <label for="fields.description.{{$index}}" class="form-label">डाटा
                                                            *</label>
                                                        <textarea name="fields[{{$index}}][description]"
                                                                  id="fields.description.{{$index}}" required cols="10"
                                                                  rows="1"
                                                                  class="form-control ckEditor @error('description') is-invalid @enderror">{{old('fields.'.$index.'.description', $field->description)}}</textarea>
                                                        @error('description')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                        @enderror
                                                    </div>
                                        ...
                                        <div class="main">
                                            ...
                                            <div class="row border-bottom mb-2">
                                                <div class="col-md-12 mb-2">
                                                    ...
                                                    <input type="text" name="fields[{{ count(old('fields',$form->formDocumentFormats))}}][title]"
                                                           class="form-control"
                                                           id="documents" multiple/>
                                                </div>

                                                <div class="col-md-12 mb-2">
                                                    <label for="fields.description.{{ count(old('fields',$form->formDocumentFormats))}}" class="form-label">डाटा
                                                        *</label>
                                                    <textarea name="fields[{{ count(old('fields',$form->formDocumentFormats))}}][description]"
                                                              id="fields.description.{{ count(old('fields',$form->formDocumentFormats))}}" required cols="10"
                                                              rows="1"
