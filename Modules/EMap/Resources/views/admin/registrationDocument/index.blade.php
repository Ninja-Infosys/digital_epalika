            ...
            <div class="card p-0">
                <div class="card-header search-card">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="header-title mb-0">दरखस्त कागजातहरू सूची</h4>
                        @can('mapFee_create')
                            <a href="{{ route('emap.admin.registrationDocument.create') }}" class="btn btn-sm btn-outline-primary">
                            ...
                            <tbody>
                                @forelse($registrationDocuments as $key=>$registrationDocument)
                                    <tr>
                                        ...
                                        <td> {{ Str::limit(strip_tags($registrationDocument->description ??''), 100, '..')}}
                                        <td/>
                                        <td class="d-flex">
                                            @can('registrationDocument_edit')
                                                <a data-bs-type="edit"
                                                    href="{{ route('emap.admin.registrationDocument.edit', $registrationDocument) }}"
                                            ...
                                            @can('registrationDocument_access')
                                                <a href="{{ route('emap.admin.registrationDocument.show', $registrationDocument) }}"
                                            ...
                                            <form
                                                action="{{ route('emap.admin.registrationDocument.destroy', $registrationDocument) }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
                                                @can('registrationDocument_delete')
