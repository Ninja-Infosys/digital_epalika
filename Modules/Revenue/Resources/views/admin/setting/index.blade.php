    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('admin.revenue.setting.store')}}" method="post"
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <select name="land_measurement_id" id="land_measurement_id"
                                    ...
                                    @foreach($unitTypes as $unitType)
                                        <option
                                            value="{{$unitType->id}}" {{$unitType->id == old('land_measurement_id',$revenueSetting->land_measurement_id ?? '') ? 'selected' : ''}}>{{$unitType->title}}</option>
                                    @endforeach
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <select name="land_measurement_standard_id" id="land_measurement_standard_id"
                                    ...
                                    @foreach($units as $unit)
                                        <option
                                            value="{{$unit->id}}" {{$unit->id == old('land_measurement_standard_id',$revenueSetting->land_measurement_standard_id ?? '') ? 'selected' : ''}}>{{$unit->title}}</option>
                                    @endforeach
                                </select>
